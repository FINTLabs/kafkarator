...
import no.fintlabs.operator.AivenKafkaAclCrd;
...
public class SecretService {
    ...
    public void createSecretIfNeeded(Context<AivenKafkaAclCrd> context, AivenKafkaAclCrd resource, String username, String password, String accessKey, String accessCert, String aclId) {
        if (context.getSecondaryResource(Secret.class).isEmpty()) {
            ...
            stringData.put(resource.getMetadata().getName() + ".aiven.username", username);
    ...
    public String getSecretIfExists(Context<AivenKafkaAclCrd> context, AivenKafkaAclCrd resource, String key) {
        if (context.getSecondaryResource(Secret.class).isPresent()) {
            ...
            return new String(java.util.Base64.getDecoder().decode(encodedSecret));
        }
        return null;
    }
